...
Owner: _(you)_ · Baselined on branch **`develop`** · Date: 2026-07-03
...
> ⚠️ **Branch discipline.** `develop` is the integration trunk (as of 2026-06-13).
> `main` is frozen at 2026-05-03 — **114 commits / ~40k lines behind**. Do all work on `develop`.
> Don't trust anything read from `main`; its README/CLAUDE.md are stale.
...
develop's README and CLAUDE.md are current. Small cleanups + one reconciliation:

- [ ] **Test-count drift.** README says "280 tests"; CLAUDE.md says "375 tests in 78 suites". **Actual `swift test` = 397 tests / 83 suites** (measured 2026-07-03). Update both docs; note `make test-all` adds the App-target `AppTests/` on top.
...
- [ ] `main`'s README/CLAUDE.md are badly stale — decide whether `main` gets retired/merged so newcomers don't clone the wrong branch (see §4).
...
- [ ] Decide `main`'s fate: merge `develop → main` to make trunk current, or document develop-as-default and update the remote default branch. Right now a fresh clone lands on stale `main`.
